...
public class EmulatorVT100 extends Emulator {
	...
	// public EmulatorVT100(Term term, InDataHandler4 in) {
	// super(term, in);
	// }
...
//		byte b;
	...
	private boolean bb() throws IOException{
//		byte b;
		b = getChar();
		if (b == 'B') {
			b = getChar();
...
//		return false;
	...
	private boolean ee() throws IOException{
//		byte b;
		b = getChar();
		if (b == '1') {
			b = getChar();
			if (b == 'l' || b == 'h') {
				...
				if (b == 0x1b) {
					b = getChar();
					if (b == '>' || b == '=') { //TODO rmkx , leave'keybroad_transmit' mode smkx , enter'keyborad_transmit'mode
						return true;
					}
				}
			} else if (b == 'h') {
			...
			b = getChar();
			if (b == 'h') { // smam
				//TODO System.out.println("turn on automatic magins");
				return true;
			} else if (b == 'l') { // rmam
				// TODO System.out.println("turn off automatic magins");
				return true;
			}
			pushChar(b);
			b = '7';
...
// private static byte[] RIGHT = { (byte) 0x1b, (byte) /* 0x5b */0x4f, (byte)
// 0x43 };
// private static byte[] LEFT = { (byte) 0x1b, (byte) /* 0x5b */0x4f, (byte)
// 0x44 };
 ...
 * sgr=\E[0%?%p1%p6
 * %|%t;1%;%?%p2%t;4%;%?%p1%p3%|%t;7%;%?%p4%t;5%;m%?%p9%t\016%e\017%;$<2>,
 * sgr0=\E[m\017$<2>, smacs=^N, smam=\E[?7h, smkx=\E[?1h\E=, smso=\E[7m$<2>,
